...
pub fn uninstall(assume_yes: bool) -> Result<()> {
    ...
    plan.execute()?;
    eprintln!(
        "Attached was uninstalled. Managed local state was deleted; the shared OS encryption credential was preserved so custom state directories remain decryptable."
    );
    Ok(())
}
...
impl UninstallPlan {
    ...
    fn execute_with_store(&self, store: &dyn local_encryption::MasterKeyStore) -> Result<()> {
        ...
        local_encryption::with_key_coordination(|| {
            ...
            // The keyring selector is shared by every state directory, including
            // custom directories that uninstall cannot discover. Preserve the key
...
fn confirm_uninstall(
    ...
    writeln!(
        output,
        "The shared OS encryption credential is preserved because custom state directories cannot be discovered."
    )?;
    writeln!(
        output,
...
mod tests {
    ...
    #[test]
    fn uninstall_does_not_touch_the_shared_keyring_credential() {
        let root = crate::test_support::canonical_tempdir();
        let executable = executable(root.path());
